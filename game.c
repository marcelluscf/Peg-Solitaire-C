...
void print_move_validity_string(enum move_validity move_validity)
{
	...
	{
		case VALID: printf("Move valid"); break;
		case MALFORMED_INPUT: printf("Malformed input"); break;
		case INVALID_START_POSITION: printf("Invalid start position"); break;
		case INVALID_END_POSITION: printf("Invalid end position"); break;
		case START_NOT_PEG: printf("Start position must be a peg"); break;
		case END_NOT_HOLE: printf("End position must be a hole"); break;
		case NOT_SAME_ROW_COLUMN: printf("Positions must lie on the same row/column and be separated by one cell"); break;
		case NO_PEG_TO_JUMP: printf("No peg between the two positions"); break;
	}
}
